<template>
  <Layout>
    <!--Hero-->
    <section class="inner-hero" data-aos="fade-in" data-aos-duration="1000">
      <div class="container">
        <div class="row text-center">
          <div class="col-lg-12">
            <h1
              data-aos="fade-in"
              data-aos-duration="1000"
              data-aos-delay="500"
            >
              Chapman Murray Associates Architects, Inc.
            </h1>
            <p
              data-aos="fade-in"
              data-aos-duration="1000"
              data-aos-delay="1000"
            >
              Website Design, Development &amp; SEO Proposal
            </p>
          </div>
        </div>
      </div>
    </section>

    <!--Intro-->
    <section class="intro">
      <div class="container">
        <div class="row cards">
          <div class="col-lg-1">&nbsp;</div>
          <div class="col-lg-10">
            <div class="card">
              <h4>
                Infused Agency &amp;
                <strong>Chapman Murray Associates Architects, Inc.</strong>
              </h4>
              <p>
                Based on our previous discussion, we've established that you're
                ready to enter the next stage of growth and further stabilize
                your business with a modern website. The best strategy to
                achieve your growth goals for 2020 and beyond involves a 3-part
                plan:
              </p>
              <ol>
                <li>
                  We'll start with a discovery deep dive, which will accomplish
                  two key things. First, we'll ensure the new website design's
                  branding and positioning is consistent across the website.
                  Secondly, we'll go into which parts of your business you want
                  to highlight not only to maximize your user's experience, but
                  also to help maximize your own company's revenue.
                </li>
                <li>
                  We'll create a customized website design derived from our
                  discussion based on your most important business goals. Local
                  SEO keyword implementation on the home page will be key, so
                  customers Googling for what you offer are able to find you.
                  This design will be implemented flawlessly across your website
                  into a live, functioning site, ensuring a great
                  user-experience.
                </li>
                <li>
                  Once launched and if you'd like to exploer this avenue, we
                  also specialize in building city-based landing pages that act
                  as sales funnels, and rank very high in Google for the search
                  terms your customers are searching for. This would help
                  estable yourself more within the area, and help bring in new
                  projects.
                </li>
              </ol>
              <p>
                To keep things flowing, it's imperative that we do this in a
                phased approach as you will see below.
              </p>
            </div>
          </div>
          <div class="col-lg-1">&nbsp;</div>
        </div>
      </div>
    </section>

    <!--Featured-->
    <section class="featured" data-aos="fade-in" data-aos-duration="1000">
      <div class="container">
        <div class="row text-center">
          <div class="col-lg-12">
            <h2>Local, Real Results</h2>
            <h4>Cass-A-Bella Construction</h4>
            <p>
              "Great Job, WELL DONE. I'm very please with the service and the
              design of my new web site. If you ever need a new web site I do
              recommend Infused Agency for their services. JD keep up the great
              work. Thank you."
              <br />
              <em>- Mr. Duarte, Cass-A-Bella Construction</em>
              <br />
              <br />
              <a
                target="_blank"
                href="https://www.google.com/search?q=infused+agency&oq=infused+agency&aqs=chrome..69i57j0j46j69i60j69i61l3j69i65.2615j0j7&sourceid=chrome&ie=UTF-8#lrd=0x89d3507111975161:0x18a1198be6280a70,1,,,"
                >Read More 5 Star Google Reviews</a
              >
            </p>
          </div>
        </div>
        <!--Image-->
        <div class="row">
          <div class="col-lg-1">&nbsp;</div>
          <div class="col-lg-10">
            <g-image
              alt="small &amp; local business website design in niagara ON"
              src="~/img/niagara-web-design.jpg"
              class="img-fluid cass"
            />
          </div>
          <div class="col-lg-1">&nbsp;</div>
        </div>
        <br />
        <div class="row">
          <div class="col-lg-6" data-aos="fade-up" data-aos-duration="1000">
            <h3>Before</h3>
            <ul>
              <li>Zero sales from old website</li>
              <li>Poor trust and credibility to the public</li>
              <li>Wanted more credibility and more clients</li>
            </ul>
          </div>
          <div
            class="col-lg-6"
            data-aos="fade-up"
            data-aos-duration="1000"
            data-aos-deloay="500"
          >
            <h3>After</h3>
            <ul>
              <li>Modern, blazing fast website designed and developed</li>
              <li>
                Organic Google Search views
                <strong>went up over 320%.</strong>
              </li>
              <li>
                Google Map views
                <strong>went up almost 100%.</strong>
              </li>
              <li>
                Website visits
                <strong>went up over 250%.</strong>
              </li>
              <li>
                Within the first month,
                <strong>three new clients.</strong>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <br />
      <div class="container">
        <div class="row text-center">
          <div class="col-lg-12">
            <a target="_blank" href="https://cassabellaconstruction.com/">
              <button type="submit" class="btn btn-primary">
                View Website
              </button>
            </a>
          </div>
        </div>
      </div>

      <br />
      <br />

      <div class="container">
        <div class="row">
          <div class="col-lg-12">
            <h3>Ranking #1 on Google Maps for Multiple Keywords</h3>
          </div>
          <div class="row">
            <div class="col-lg-4">
              <img
                class="img-fluid mx-auto d-block"
                src="~../img/cass-1.jpg"
                alt="1"
              />
            </div>
            <div class="col-lg-4">
              <img
                class="img-fluid mx-auto d-block"
                src="~../img/cass-2.jpg"
                alt="2"
              />
            </div>
            <div class="col-lg-4">
              <img
                class="img-fluid mx-auto d-block"
                src="~../img/cass-3.jpg"
                alt="3"
              />
            </div>
          </div>
        </div>
      </div>
    </section>

    <!--Pricing-->

    <section id="pricing">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <h2>Key Take-Aways</h2>
            <ul>
              <li>Modern design</li>
              <li>Responsive (mobile-friendly)</li>
              <li>Help to rank well in Google</li>
              <li>Easy-to-navigate user experience</li>
              <li>Proposed Sitemap:</li>
              <ul>
                <li>Home</li>
                <li>Firm Profile - encapsulating all sub pages into one</li>
                <li>
                  Portfolio - listed by categories and projects within each
                  category, corresponding picture galleries to go along with
                  each project
                </li>
                <li>Awards & Publications</li>
                <li>Contact</li>
              </ul>
            </ul>
            <h2>Project Investment</h2>
            <p>
              Your website will have a modern web design and will be developed
              on a platform that no other agency has access to, which makes us a
              very modern & unique choice. We choose this platform to develop
              all of our websites as it's:
            </p>
            <ul>
              <li>Extremely fast</li>
              <li>Modern</li>
              <li>There are no hosting fees!</li>
              <li>It's non-hackable and extremely secure</li>
              <li>
                Rank extremely well in Google, leading to more revenue and
                exposure for your firm
              </li>
            </ul>
          </div>
        </div>
        <div class="row">
          <div class="col-md-12">
            <h2>Website Pricing</h2>
            <p>
              Three affordable packages to choose from. First month upfront,
              website launch within a month.
            </p>
          </div>
        </div>
        <div class="row">
          <div class="col-md-4">
            <div class="heading">
              <h2>Beginner</h2>
            </div>
            <div class="card">
              <h3><sup>$</sup>164.99 / mo</h3>
              <h6>For 1 year</h6>
              <p>1 page</p>
              <ul>
                <li>Custom design</li>
                <li>Includes hosting</li>
              </ul>
            </div>
          </div>
          <div class="col-md-4">
            <div class="heading">
              <h2>Professional</h2>
            </div>
            <div class="value">
              <h4>Best value, most popular (includes SEO)</h4>
            </div>
            <div class="card">
              <h3><sup>$</sup>299.99 / mo</h3>
              <h6>For 1 year</h6>
              <p>Up to 20 pages</p>
              <ul>
                <li>Everything from Beginner package</li>
                <li>Home page optimized for local keywords</li>
                <li>
                  Optimized for SEO (meta titles, descriptions, home page with
                  localized SEO)
                </li>
                <li>SSL security certificate</li>
                <li>
                  Create / optimize Google My Business listings for local SEO
                </li>
                <li>Contact form to collect leads</li>
              </ul>
            </div>
          </div>
          <div class="col-md-4">
            <div class="heading">
              <h2>Enterprise</h2>
            </div>
            <div class="card">
              <h3><sup>$</sup>999 / mo+</h3>
              <h6>For 1 year</h6>
              <p>Unlimited pages</p>
              <ul>
                <li>Everything from Professional package</li>
                <li>E-commerce</li>
                <li>Membership area</li>
                <li>Search functionality</li>
                <li>Complex programming logic</li>
              </ul>
            </div>
          </div>
        </div>

        <div class="row">
          <div class="col-md-12">
            <h2>Next Steps</h2>
            <ol>
              <li>Internal buy-in</li>
              <li>
                Contract signing and initial deposit (first and last month
                upfront)
              </li>
              <li>We'll get started!</li>
            </ol>
          </div>
        </div>
      </div>
    </section>
  </Layout>
</template>

<style scoped lang="scss">
@import "../scss/_base.scss";

// Intro / Pricing
.intro,
.pricing {
  background: $light-grey;
  padding-top: 100px;
  padding-bottom: 100px;
  @include bp-tablet {
    padding-top: 60px;
    padding-bottom: 60px;
  }
  .cards {
    .col-lg-4 {
      display: flex;
    }
    .card {
      width: 100%;
      padding: 30px;
      border: none;
      box-shadow: 0px 20px 100px #edeff2;
      transition: 0.2s;
      &:focus,
      &:hover {
        box-shadow: 0px 20px 100px #c7c7c7;
        transform: translate3D(0, -5px, 0);
        transition: 0.2s;
      }
      @include bp-tablet {
        width: 100%;
        margin-bottom: 20px;
      }
      h4 {
        margin-bottom: 10px;
        margin-top: 15px;
      }
    }
  }
}

.main-project {
  padding-top: 100px;
  padding-bottom: 100px;
  @include bp-tablet {
    padding-top: 60px;
    padding-bottom: 60px;
  }
  img {
    margin-bottom: 60px;
  }
}

.featured {
  padding-top: 100px;
  padding-bottom: 100px;
  @include bp-tablet {
    padding-top: 60px;
    padding-bottom: 60px;
  }
}

// Pricing
#pricing {
  background: $light-grey;
  padding-top: 100px;
  padding-bottom: 100px;
  @include bp-tablet {
    padding-top: 60px;
    padding-bottom: 60px;
  }
  .heading {
    background-image: linear-gradient(to right, #412caf, #0671bc);
    text-align: center;
    h2 {
      color: $white;
      margin-bottom: 0px;
      padding-top: 15px;
      padding-bottom: 15px;
    }
  }
  .value {
    background: #2ab347;
    text-align: center;
    padding-top: 10px;
    padding-bottom: 10px;
    h4 {
      color: $white;
      margin-bottom: 0px;
    }
  }
  .card {
    @include bp-tablet-sm {
      margin-bottom: 20px;
    }
    text-align: center;
    padding: 15px;
    h3 {
      margin-bottom: 0px;
    }
    p {
      font-size: 16px;
      margin-bottom: 10px;
    }
    ul {
      list-style: none;
      margin-left: -40px;
      li {
        border-bottom: 1px solid $light-blue;
        padding-top: 5px;
        padding-bottom: 5px;
      }
    }
  }
}
//FAQ
#faq {
  padding-top: 100px;
  padding-bottom: 100px;
  @include bp-tablet {
    padding-top: 60px;
    padding-bottom: 60px;
  }
}
// CTA
.pdf {
  background: linear-gradient(to right, #2caf73, #0671bc);
  padding-top: 60px;
  padding-bottom: 60px;
  @include bp-tablet {
    text-align: center;
  }
  .row {
    align-items: center;
  }
  h2,
  p {
    color: $white;
  }
  h2 {
    margin-bottom: 0px;
    @include bp-tablet {
      margin-bottom: 20px;
    }
  }
  .btn-primary {
    background: $white;
    color: $dark-blue;
  }
}
</style>

<script>
export default {
  metaInfo: {
    title: "Chapman Murray Associates Architects, Inc. - Proposal",
    meta: [
      {
        key: "description",
        name: "description",
        content:
          "Please view Chapman Murray Associates Architects, Inc.'s proposal.",
      },
    ],
  },
};
</script>
